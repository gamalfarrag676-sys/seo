...
export default function Admin() {
 ...
 <p className="text-gray-500 text-sm">إدارة المستخدمين وإعدادات النظام</p>
 </div>
 </div>

 <button
 onClick={() => { fetchUsers(); fetchApiKeys(); }}
 className="p-2 rounded-xl bg-blue-500 hover:bg-blue-50 text-blue-700 transition-all"
 >
 ...
 {/* Admin Tabs */}
 <div className="flex gap-2 mb-6 p-1.5 bg-gray-50 rounded-2xl border border-gray-200">
 <button
 ...
 <div className="text-xs text-gray-500">المستخدمين</div>
 </div>
 <div className="card p-4 text-center">
 ...
 <div className="text-xs text-gray-500">المديرين</div>
 </div>
 <div className="card p-4 text-center">
 <Package className="w-6 h-6 text-pink-400 mx-auto mb-2" />
 <div className="text-2xl font-bold text-gray-900">{users.reduce((a, u) => a + (u.products_used || 0), 0)}</div>
 <div className="text-xs text-gray-500">المنتجات</div>
 </div>
 <div className="card p-4 text-center">
 <FileText className="w-6 h-6 text-emerald-400 mx-auto mb-2" />
 <div className="text-2xl font-bold text-gray-900">{users.reduce((a, u) => a + (u.articles_used || 0), 0)}</div>
 <div className="text-xs text-gray-500">المقالات</div>
 </div>
 </div>
 ...
 className="w-full bg-gray-50 border border-gray-200 rounded-xl pr-10 pl-4 py-3 text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-blue-500 outline-none"
 />
 </div>
 ...
 <th className="text-right text-xs font-medium text-gray-500 p-4">المستخدم</th>
 <th className="text-center text-xs font-medium text-gray-500 p-4">الدور</th>
 <th className="text-center text-xs font-medium text-gray-500 p-4">المنتجات</th>
 <th className="text-center text-xs font-medium text-gray-500 p-4">المقالات</th>
 <th className="text-center text-xs font-medium text-gray-500 p-4">إجراءات</th>
 </tr>
 </thead>
 <tbody>
 ...
 <div>
 <div className="text-gray-900 font-medium text-sm">{user.full_name || 'بدون اسم'}</div>
 <div className="text-gray-400 text-xs" dir="ltr">{user.email}</div>
 </div>
 </div>
 </td>
 ...
 className={`px-3 py-1 rounded-full text-xs font-bold ${user.role === 'admin' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-gray-200 text-gray-500'}`}
 >
 ...
 className="w-16 bg-gray-100 border border-gray-300 rounded px-2 py-1 text-gray-900 text-sm text-center"
 />
 ) : (
 <span className="text-gray-900">
 <span className="text-pink-400">{user.products_used}</span>
 <span className="text-gray-400">/{user.products_limit}</span>
 </span>
 )}
 </td>
 <td className="p-4 text-center">
 ...
 <span className="text-gray-400">/{user.articles_limit}</span>
 </span>
 )}
 </td>
 <td className="p-4">
 ...
 className="p-2 rounded-lg bg-gray-200 text-gray-500 hover:bg-gray-200"
 >
 <X className="w-4 h-4" />
 </button>
 </>
 ...
 className="p-2 rounded-lg bg-blue-50 text-blue-700 hover:bg-blue-500"
 title="تعديل الحدود"
 >
 ...
 <p className="text-gray-500 text-sm">أنشئ حساب جديد للنظام</p>
 </div>

 <div className="space-y-4">
 <div>
 <label className="text-sm font-medium text-gray-600 mb-1 block">الاسم الكامل *</label>
 <div className="relative">
 <input
 type="text"
 value={newUserForm.fullName}
 onChange={(e) => setNewUserForm({ ...newUserForm, fullName: e.target.value })}
 className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-emerald-500/50 outline-none"
 ...
 className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-16 pr-4 py-3 text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-emerald-500/50 outline-none"
 placeholder="••••••••"
 dir="ltr"
 />
 ...
 <div>
 <label className="text-sm font-medium text-gray-600 mb-1 block">الدور</label>
 <select
 value={newUserForm.role}
 onChange={(e) => setNewUserForm({ ...newUserForm, role: e.target.value as 'user' | 'admin' })}
 className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-900 focus:ring-2 focus:ring-emerald-500/50 outline-none"
 >
 ...
 <div>
 <label className="text-sm font-medium text-gray-600 mb-1 block">حد المنتجات</label>
 <input
 type="number"
 ...
 <div>
 <label className="text-sm font-medium text-gray-600 mb-1 block">حد المقالات</label>
 <input
 type="number"
 ...
 className="w-full bg-emerald-600 hover:bg-emerald-500 text-gray-900 font-bold py-3 rounded-xl transition-all shadow-lg shadow-emerald-900/20 active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-50"
 >
 {creatingUser ? (
 ...
 <div className="card p-6 max-w-lg mx-auto">
 <div className="text-center mb-6">
 <div className="inline-flex items-center justify-center p-3 bg-blue-500 rounded-2xl ring-1 ring-blue-500 mb-4">
 <Key className="w-8 h-8 text-blue-600" />
 </div>
 <h2 className="text-xl font-bold text-gray-900">مفاتيح الذكاء الاصطناعي</h2>
 <p className="text-gray-500 text-sm">هذه المفاتيح يستخدمها جميع المستخدمين</p>
 </div>
 ...
 className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-blue-500 outline-none font-mono text-sm"
 placeholder="AIzaSy..."
 dir="ltr"
 />
 <button
 ...
 disabled={savingKeys}
 className="w-full bg-blue-600 hover:bg-blue-500 text-gray-900 font-bold py-3 rounded-xl transition-all shadow-lg shadow-purple-900/20 active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-50"
 >
 {savingKeys ? (
