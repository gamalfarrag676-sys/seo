...
export function ImageUpload({ onImageSelect, selectedImage, className }: ImageUploadProps) {
 ...
 : "border-gray-300 hover:border-emerald-500/50 hover:bg-gray-100 bg-gray-100/20"
 )}>
 {selectedImage ? (
 ...
 {isOptimizing && (
 <div className="absolute inset-0 z-20 flex items-center justify-center bg-gray-50/70 -sm rounded-xl">
 <div className="flex flex-col items-center gap-2">
 <Loader2 className="w-8 h-8 text-emerald-400 animate-spin" />
 <span className="text-sm text-gray-700">جاري تحسين الصورة...</span>
 </div>
 </div>
 )}
 ...
 {optimizationStats && (
 <div 
 className="absolute md:static bottom-0 w-full md:w-56 bg-gray-50/90 -md p-3 rounded-xl border border-emerald-500/20 shadow-xl flex flex-col gap-2 animate-in slide-in-from-bottom-2"
 ...
 className="mt-2 w-full py-2 bg-emerald-500 hover:bg-emerald-600 text-gray-900 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-colors"
 >
 <Download size={14} />
 تحميل الصورة
 </button>
 ...
 <button 
 onClick={removeImage}
 className="absolute top-2 right-2 p-2 bg-red-500/80 hover:bg-red-500 text-gray-900 rounded-full transition-colors -sm shadow-lg z-10"
 >
 <X size={16} />
 </button>
 ...
 <div className="w-16 h-16 rounded-full bg-gray-200 flex items-center justify-center mx-auto group-hover:scale-110 transition-transform duration-300">
 <Upload className="w-8 h-8 text-gray-500 group-hover:text-emerald-400" />
 </div>
 <div>
