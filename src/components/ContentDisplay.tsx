...
const SectionCard = ({ title, icon: Icon, content, isMarkdown = false, richCopy = false }: { title: string, icon: any, content: string | undefined, isMarkdown?: boolean, richCopy?: boolean }) => {
  ...
  return (
    <div className="glass-card p-6 mb-6 animate-fade-in relative group">
      ...
      <div className="text-slate-200 leading-relaxed text-right md-content">
        {isMarkdown ? (
          <ReactMarkdown
            components={{
          ...
          <div className="whitespace-pre-wrap font-mono text-sm bg-slate-900/50 p-4 rounded-lg border border-slate-800">
            {content}
          </div>
        )}
      </div>
