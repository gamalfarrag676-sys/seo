...
export const BulkGenerator: React.FC = () => {
 ...
 <p className="text-sm text-gray-500 mt-1">ارفع ملف CSV لتوليد محتوى عشرات المنتجات دفعة واحدة.</p>
 </div>
 ...
 className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg text-sm font-bold transition-colors flex flex-1 justify-center items-center gap-2"
 >
 ...
 className="px-4 py-2 bg-blue-500 hover:bg-blue-50 text-blue-700 border border-blue-200 rounded-lg text-sm font-bold transition-colors flex flex-1 justify-center items-center gap-2"
 >
 ...
 className="btn btn-primary px-6 py-2 rounded-lg text-sm flex-1 md:flex-none justify-center"
 >
 ...
 <span className="font-bold text-blue-600">جاري المعالجة: {progress.current}</span>
 ...
 <h3 className="font-bold text-gray-900">قائمة المنتجات</h3>
 ...
 {items.map((item) => (
 <tr key={item.id} className="hover:bg-gray-100/20 transition-colors">
 <td className="px-4 py-3 w-10">
 {item.status === 'pending' && <div className="w-2 h-2 rounded-full bg-gray-200" />}
 {item.status === 'processing' && <Loader2 className="w-4 h-4 text-blue-600 animate-spin" />}
 ...
 <span className="px-2 py-1 bg-gray-100 rounded text-xs">{item.keyword}</span>
 </td>
 <td className="px-4 py-3 text-gray-500">
 {item.result ? (
 ...
 <h3 className="text-lg font-bold text-gray-600 mb-2">القائمة فارغة</h3>
