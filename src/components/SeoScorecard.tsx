      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-right">
            <span className="text-sm font-medium text-slate-200">{categoryNames[category]}</span>
          ...
          {categoryChecks.map((check) => (
            ...
            >
              ...
              <div>
                ...
                {check.suggestion && (
                  <p className="text-slate-400 mt-0.5">💡 {check.suggestion}</p>
                )}
              </div>
              <span className="mr-auto text-slate-500 shrink-0">+{check.score}</span>
            </div>
          ))}
        </div>
      )}
...
export function SeoScorecard({ result, isLoading }: SeoScorecardProps) {
  ...
  return (
    <div className="bg-slate-800/50 backdrop-blur-md border border-slate-700 rounded-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="p-5 border-b border-slate-700/50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-base text-slate-200">تقرير SEO المتقدم</h3>
          ...
          <div className="flex items-center gap-3">
            <div className="text-left">
              <div className="text-2xl font-black text-slate-200">{result.score}</div>
              <div className="text-xs text-slate-400">من {result.maxScore}</div>
            </div>
            <div className={clsx(
              "w-14 h-14 rounded-xl flex items-center justify-center font-black text-2xl text-white shadow-lg bg-gradient-to-br",
      ...
      <div className="p-4 bg-slate-900/50 border-t border-slate-700/50">
        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="p-2 rounded-lg bg-emerald-500/10">
            ...
            <div className="text-xs text-slate-400">نجح</div>
          </div>
          <div className="p-2 rounded-lg bg-amber-500/10">
            <div className="text-lg font-bold text-amber-400">{warningCount}</div>
            <div className="text-xs text-slate-400">تحذير</div>
          </div>
